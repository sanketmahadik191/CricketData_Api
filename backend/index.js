...
app.get('/api/cricket', async (req,res)=>{ // Added async here
  ...
  fs.access(`${year.year}.txt`,async (err)=>{
    if(err){
  try {
    ...
    try {
      fs.writeFileSync(`${year.year}.txt`, JSON.stringify(combinedData));
      console.log("Data added synchronously");
    } catch (err) {
    ...
    res.status(500).send('Error generating data');
     ...
     const dataAll = fs.readFileSync(`${year.year}.txt`);
      res.send(dataAll)
  }
})
});
